...
bool MiniPatch::FindPatchRectify(CVD::ImageRef &irPos,  CVD::BasicImage<CVD::byte> &im, int nRange, std::vector<CVD::ImageRef> &vCorners)
{
	int nBestSSD = maxStereoSSD + 1;
	ImageRef irBest;
	//ImageRef irBBoxTL = irPos - ImageRef(nRange, nRange);
	//ImageRef irBBoxBR = irPos + ImageRef(nRange, nRange);
	vector<ImageRef>::iterator i;
	...
	for(i = vCorners.begin(); i!=vCorners.end(); i++)
	{
		...
		//if(i->y > irBBoxBR.y)
		//	break;
		if(irPos.y == i->y || irPos.y == i->y-1 || irPos.y == i->y+1)
		//if(irPos.y == i->y)
		{
			int nSSD = SSDAtPoint(im, *i);
	...
	// why a not detail boundary check is needed?
	if(nBestSSD < maxStereoSSD && irBest.x + mnHalfPatchSize < im.size().x)
    {
		irPos = irBest;
...
int MiniPatch::maxStereoSSD = 39999;
