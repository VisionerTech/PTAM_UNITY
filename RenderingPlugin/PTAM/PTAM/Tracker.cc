...
void Tracker::TrackForInitialMap_stereo(CVD::Image<CVD::byte> &imFrame_L, CVD::Image<CVD::byte> &imFrame_R)
{
	...
	if(mnInitialStage == TRAIL_TRACKING_NOT_STARTED) 
    {
		...
		if(mbUserPressedSpacebar)  // First spacebar = this is the first keyframe
		{
			...
			cv::Mat cvd_image(640, 480, CV_8UC1, (unsigned char*)imFrame_L.data());
			cv::Mat cvd_image_right(640, 480, CV_8UC1, (unsigned char*)imFrame_R.data());

			int test = (int)mCurrentKF.aLevels[0].im.size().x;
			string debug_msg = std::to_string(test);
			cv::putText(cvd_image, "DEBUG: " + debug_msg, cvPoint(20, 10), cv::FONT_HERSHEY_PLAIN, 1.0, cvScalar(255, 0, 0, 255), 1, CV_AA);

			test = (int)mCurrentKF.aLevels[0].vCandidates.size();
			debug_msg = std::to_string(test);
			cv::putText(cvd_image, "DEBUG: " + debug_msg, cvPoint(20, 50), cv::FONT_HERSHEY_PLAIN, 1.0, cvScalar(255, 0, 0, 255), 1, CV_AA);


			//cv::imshow("1", cvd_image);
			//cv::imshow("2", cvd_image_right);
			//cv::waitKey();
			...
			//cv::imshow("3", cvd_image);
			//cv::imshow("4", cvd_image_right);
			//cv::waitKey();
			...
			//cv::imshow("5", cvd_image);
			//cv::imshow("6", cvd_image_right);
			//cv::waitKey();
			...
			//cv::imshow("7", cvd_image);
			//cv::imshow("8", cvd_image_right);
			//cv::waitKey();
			...
			//cv::imshow("9", cvd_image);
			//cv::imshow("10", cvd_image_right);
			//cv::waitKey();
			...
			{
				//cv::imshow("11", cvd_image);
				//cv::imshow("12", cvd_image_right);
				//cv::waitKey();
			...
			//cv::imshow("13", cvd_image);
			//cv::imshow("14", cvd_image_right);
			//cv::waitKey();
		}
